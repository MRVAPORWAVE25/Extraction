...
(function(){
  ...
  */
  (function setupVisitorCounters(){
    ...
    // increment visits once per browser (persisted)
    try {
      const key = 'site_visits_v1';
      let visits = Number(localStorage.getItem(key) || '0');
      // Only increment if we haven't incremented this session (session flag)
      if (!sessionStorage.getItem('visit_counted_v1')) {
        visits = visits + 1;
        localStorage.setItem(key, String(visits));
        sessionStorage.setItem('visit_counted_v1', '1');
      }
      totalEl.textContent = String(localStorage.getItem(key) || visits || 0);
    } catch (e) {
