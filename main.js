...
/* Status strip: live time and battery updates (12-hour clock; revealed after splash wipe; SVG battery icon)
   + visitor counters (visits & active users) using BroadcastChannel + heartbeat.
*/
(function(){
  ...
  const visitorBox = document.getElementById('visitor-stats');

  // hide status strip until the splash has been wiped/entered
  try { if (statusStrip) statusStrip.style.display = 'none'; } catch (e){}
  ...
  function showStatusStripIfEntered(){
    try {
      ...
      const hasFullscreenIframe = !!document.getElementById('__search_full_iframe');
      const inSearchBeam = document.body && document.body.classList && document.body.classList.contains('search-beam');
      const inGamesView = document.body && document.body.classList && document.body.classList.contains('games-view');
      const locPath = (location && location.pathname) ? location.pathname.replace(/\/+$/, '') : '';
      const onHomepage = (locPath === '' || locPath === '/' || locPath.endsWith('/index.html') || locPath === '/index.html');

      // control status strip & visitor box similar to previous behavior
      if (entered && !hasFullscreenIframe && !inSearchBeam && !inGamesView && onHomepage) {
        if (statusStrip) statusStrip.style.display = '';
        if (visitorBox) {
          visitorBox.setAttribute('aria-hidden','false');
          visitorBox.style.display = '';
        }
        // reveal credits button only on homepage after entering and when no fullscreen iframe is shown
        const creditsBtn = document.getElementById('credits-btn');
        if (creditsBtn) { creditsBtn.hidden = false; creditsBtn.style.display = 'inline-flex'; }
      } else {
        if (statusStrip) statusStrip.style.display = 'none';
        if (visitorBox) { visitorBox.setAttribute('aria-hidden','true'); visitorBox.style.display = 'none'; }
        const creditsBtn = document.getElementById('credits-btn');
        if (creditsBtn) { creditsBtn.hidden = true; creditsBtn.style.display = 'none'; }
      }

      // Also ensure the global top-left close button follows the same "only-on-homepage" rule.
      // When not on the homepage hide it; when on homepage leave its visibility to other flows.
      try {
        const globalBackBtn = document.getElementById('global-back');
        if (globalBackBtn) {
          // If we're on the homepage, do not force it hidden here (other logic can reveal it when needed).
          // If not on homepage, make sure it's hidden to match the status strip behaviour.
          if (!onHomepage) {
            globalBackBtn.hidden = true;
          } else {
            // keep existing state when on homepage (do not forcibly show it)
            // leave it as-is so other handlers (e.g. createFullscreenIframe) can reveal it when appropriate
          }
        }
      } catch (innerE) { /* swallow errors */ }
  ...
  function batterySvg(pct, charging){
    ...
    const fillWidth = Math.max(2, Math.round((pct / 100) * 18));
  ...
  if (navigator.getBattery && typeof navigator.getBattery === 'function') {
    ...
    }).catch(()=> { if (battEl) battEl.textContent = 'Battery: n/a'; });
  } else {
  ...
  try {
    if (splashImg) {
      splashImg.addEventListener('transitionend', (ev) => {
        ...
        updateTime();
        // show visitor box immediately on wipe
        showStatusStripIfEntered();
      }, { passive: true });
    }
  } catch (e){}
  ...
  /* Visitor counting:
     - increments a persistent "visits" counter stored in localStorage
     - uses BroadcastChannel to share a heartbeat ping; active users = unique channels with recent heartbeat
     - shows "visits" (total) and "active" (online in last 8s) in visitorBox
  */
  (function setupVisitorCounters(){
    if (!visitorBox) return;
    // build DOM
    visitorBox.innerHTML = '<div class="v-item"><span style="opacity:0.75">Visits</span><span id="v-total" class="v-count">0</span></div>' +
                           '<div class="v-item"><span style="opacity:0.75">Active</span><span id="v-active" class="v-count">0</span></div>';
    const totalEl = visitorBox.querySelector('#v-total');
    const activeEl = visitorBox.querySelector('#v-active');

    // increment visits once per browser (persisted) and keep total refreshed across tabs
    try {
      const key = 'site_visits_v1';
      // Only increment once per session
      if (!sessionStorage.getItem('visit_counted_v1')) {
      ...
      // Function to update the displayed total by reading localStorage (keeps UI in sync across tabs)
      function refreshTotal() {
        try {
          const v = String(localStorage.getItem(key) || '0');
      ...
      // keep the total in sync by polling localStorage periodically (also catches storage events)
      const TOTAL_REFRESH_MS = 2000;
      const totalRefreshId = setInterval(refreshTotal, TOTAL_REFRESH_MS);

      // listen for storage events as well to instantly update when other tabs change it
      window.addEventListener('storage', (ev) => {
        if (ev.key === key) refreshTotal();
      });

      // expose for debugging/cleanup if needed
      try { window._visit_total_refresher = { stop: ()=> clearInterval(totalRefreshId) }; } catch (e){}
    } catch (e) {
      totalEl.textContent = 'n/a';
    }

    // BroadcastChannel for heartbeat presence
    const CHANNEL = 'site-presence-v1';
    let bc = null;
    try {
      if ('BroadcastChannel' in window) bc = new BroadcastChannel(CHANNEL);
    } catch (e) { bc = null; }

    // fallback via localStorage events if no BroadcastChannel
    const fallback = !bc;
    const ID = 'id-' + Math.random().toString(36).slice(2,9);
    const PRESENCE_TTL = 8000; // ms
    const HEART_INTERVAL = 3000; // ms
    const seen = new Map(); // id -> lastTs

    function writeHeartbeat() {
      const now = Date.now();
      seen.set(ID, now);
      if (bc) {
        try { bc.postMessage({ t: now, id: ID }); } catch (e){}
      } else {
        try { localStorage.setItem('presence_ping', JSON.stringify({ t: now, id: ID })); } catch (e){}
      }
      tick();
    }
    ...
    function onMessage(msg) {
      try {
        ...
        seen.set(msg.id, msg.t || Date.now());
      } catch (e){}
    }
    ...
    if (bc) {
      ...
      window.addEventListener('storage', (ev) => {
        if (ev.key === 'presence_ping' && ev.newValue) {
          try {
    ...
    // remove stale entries and update DOM
    function tick(){
      const now = Date.now();
      // cleanup
      for (const [k,v] of Array.from(seen.entries())){
        if ((now - v) > Math.max(PRESENCE_TTL, HEART_INTERVAL*3)) seen.delete(k);
      }
      // active count excludes self if not yet counted? include self
      const activeCount = Math.max(1, Array.from(seen.keys()).length);
      activeEl.textContent = String(activeCount);
    }

    // start heartbeat
    writeHeartbeat();
    const heartId = setInterval(writeHeartbeat, HEART_INTERVAL);

    // ensure we remove our presence on unload (best-effort)
    function removePresence(){
      try {
        seen.delete(ID);
        if (bc) {
          try { bc.postMessage({ id: ID, t: Date.now(), leaving: true }); } catch (e){}
          bc.close && bc.close();
        } else {
          try { localStorage.setItem('presence_ping', JSON.stringify({ id: ID, t: Date.now(), leaving:true })); } catch (e){}
        }
      } catch (e){}
    }
    window.addEventListener('beforeunload', removePresence, { passive: true });
    window.addEventListener('pagehide', removePresence, { passive: true });

    // continuous ticker to refresh active count display
    const refreshId = setInterval(tick, 1000);

    // expose for debugging if needed
    try { window._visitor_presence = { id: ID, seen, stop: ()=>{ clearInterval(heartId); clearInterval(refreshId); removePresence(); } }; } catch (e){}
  })();
...
function createFullscreenIframe(url){
  ...
  // re-evaluate homepage-based chrome visibility after creating the iframe
  try { if (typeof window.showStatusStripIfEntered === 'function') window.showStatusStripIfEntered(); } catch(e){}

  return iframe;
}

/* Credits button behavior: open the credits page in the fullscreen iframe when clicked (homepage-only control) */
(function(){
  try {
    ...
    btn.addEventListener('click', (e) => {
      ...
      try {
        // create or reuse the fullscreen iframe to load the local credits page
        createFullscreenIframe('github-fork-files/credits.html');
      } catch (err) {
        try { window.location.href = 'github-fork-files/credits.html'; } catch (ee) {}
      }
    }, { passive: true });
  } catch (e){}
})();
...
window.addEventListener('message', (ev) => {
  try {
    ...
    // allow child frames to request the parent open an external URL in the site's fullscreen iframe
    if (msg && msg.type === 'open-url' && msg.url) {
      try {
    ...
    // allow child frames to request a modal-sized game/app preview inside the parent (keeps the games/apps preview behavior)
    if (msg && msg.type === 'open-preview' && msg.url) {
      try {
        const url = String(msg.url);
        const titleText = String(msg.title || 'Preview');

        // Reuse or create a modal similar to the games/apps preview modal (id: game-modal)
        ...
        if (!modal) {
          ...
          modal.innerHTML = `
            ...
            <div class="card" role="dialog" aria-label="Preview">
              <div class="top">
                <div class="title"></div>
                <button class="close" aria-label="Close preview">&times;</button>
              </div>
              <iframe sandbox="allow-scripts allow-same-origin" scrolling="no" title="Preview iframe"></iframe>
            </div>
          `;
        ...
        // apply comfortable preview sizing similar to the games preview
        cardEl.style.width = 'min(92vw, 900px)';
        cardEl.style.height = 'min(80vh, 720px)';
        iframe.style.width = '100%';
        iframe.style.height = '100%';

        // load URL into the modal iframe
        try {
          if (iframe._blobUrl) { URL.revokeObjectURL(iframe._blobUrl); iframe._blobUrl = null; }
        } catch (e) {}
        iframe.src = url;
        ...
        // ensure modal is visible above any existing fullscreen iframe
        try {
          modal.style.zIndex = 100000;
        } catch (e) {}
      } catch (err) {
        // fallback: open fullscreen iframe if modal creation fails
        try { createFullscreenIframe(String(msg.url)); } catch (e) {}
      }
      return;
    }
    ...
    if (msg && msg.type === 'close-preview') {
      ...
      try { window.location.href = new URL('index.html', location.href).href; } catch (e) {}
      return;
    }
